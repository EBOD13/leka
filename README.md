...
**[▶ Open the live replay viewer](https://danielesambu.com/leka/viz/)** — a recorded Nasdaq BX session and a
synthetic run, replayed through this engine: price, market depth, the order
book, engine latency percentiles, and the trade tape. The snapshots are
regenerated by CI on every push, so what you see always matches the engine
that produced it.
